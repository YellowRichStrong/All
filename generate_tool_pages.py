...
def generate_tool_page(tool):
    # Use triple double quotes to avoid issues with braces
    html = '''<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>%s - OOPEN AII</title>
    <meta name="description" content="Free online %s tool to %s">
    <meta name="keywords" content="%s, online %s, %s">
...
<body>
    ...
    <main>
        <div class="tool-container">
            <div class="tool-header">
                <div class="tool-icon-large">%s</div>
                <h2>%s</h2>
                <p class="tool-description">%s</p>
            </div>
            ...
            <div style="margin-top: 40px; background-color: #f9f9f9; padding: 30px; border-radius: 8px;">
                ...
                <ul style="list-style-type: none; padding: 0; max-width: 600px; margin: 0 auto;">
                    <li style="padding: 8px 0; border-bottom: 1px solid #eee; text-align: left;">✨ %s functionality</li>
    ...
    title_dash = title_lower.replace(' ', '-')
    description = tool['description']
    description_lower = description.lower()
    icon = tool['icon']
    
    return html % (title, title_lower, description_lower, title_dash, title_dash, title_lower, icon, title, description, title)
...
def main():
    ...
    for tool in tools:
        file_path = os.path.join(tools_dir, f"{tool['id']}.html")
        content = generate_tool_page(tool)
    ...
    print("All tool pages generated successfully!")
